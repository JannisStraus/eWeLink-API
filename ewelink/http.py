...
class HttpClient:
    ...
    async def login(
        self, **kwargs: CredentialsDict | str | None
    ) -> dict[str, str | bool | dict[str, str | bool | int | Any]]:
        self.credentials = kwargs.get("credentials") or {
        ...
        if not self.email or (
            ...
            and re.match(
                r"^(?:\+?\d{,4})?\d{10}$",
                str(self.phone).replace(" ", "").replace("(", "").replace(")", ""),
            )
        ):
        ...
        self.sign = (
            ...
            or base64.b64encode(
                hmac.new(
                    ...
                    msg=json.dumps(self.credentials).encode(),
        ...
        response = await self.session.post(
            ...
            headers={"Authorization": f"Sign {self.sign}"},
        ...
        if "error" in data:
            ...
            return data.get(
                "user",
                {"_id": "0", "clientInfo": {}, "createdAt": "1000-01-01T00:00:00.000Z"},
            )

    async def get_devices(self) -> dict[str, list[dict[str, str | int | Any]]]:
        response = await self.session.get(
            self.BASE + "/user/device",
            params={
                "lang": "en",
                "appid": constants.APP_ID,
                "ts": int(time.time()),
