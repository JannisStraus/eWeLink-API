...
class Asset:
    ...
    def __init__(
        self, url: str = None, hash: str = None, session: ClientSession = None
    ) -> None:
        if url and re.match(
            r"(?:^(?:https|http)://[A-Za-z0-9\-]+\.[A-Za-z0-9\-]+)|(?:<a?\:[a-zA-Z0-9_]+:[0-9]+>)",
            url,
            flags=re.I,
        ):
            self.url = url
            if img_hash := re.findall(
                r"^.+/(\w+)(?:\W|\s).*$", url, flags=re.IGNORECASE
            ):
