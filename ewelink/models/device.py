...
from .enumerations import DeviceType, Power
...
class Device:
    ...
    def __init__(
        self, data: dict[str, str | int | Any], state: Connection | None
    ) -> None:
        self.apikey: str | None = data.get("apikey")
        self.id: str = data.get("deviceid", "0")
        self.brand: Brand = Brand(
            name=data.get("brandName"),
            logo=Asset(
                data.get("brandLogoUrl"), session=state.http.session if state else None
            ),
        )
        self.url: str | None = data.get("deviceUrl")
        ...
        if online_time := data.get("onlineTime"):
            self.online_time = datetime.strptime(online_time, "%Y-%m-%dT%H:%M:%S.%fZ")
        if offline_time := data.get("offlineTime"):
            self.offline_time = datetime.strptime(offline_time, "%Y-%m-%dT%H:%M:%S.%fZ")
        self.params: Object = Object(data["params"], name="Params")
        self.state: Power = (
            Power[data["params"]["switches"][0]["switch"]]
            if data["params"].get("switches", None)
            else Power.unknown
        )
        self.startup: Power = (
            Power[data["params"]["startup"]]
            if data["params"].get("startup", None)
            else Power.unknown
        )
        self.pulse: Pulse = Pulse(
            state=Power[data["params"]["pulse"]]
            if data["params"].get("pulse", None)
            else Power.unknown,
        ...
        self.location: str | None = (
            data.get("location") if data.get("location") else None
        )
        self.data = data
        self._state = state
        self.type: DeviceType = DeviceType.__dict__["_value2member_map_"].get(
            int(data.get("type", 0), 16), 0
        )
        ...
        *states: Power,
        ...
        try:
            ...
            for state in states:
                ...
                if "switches" in _switch:
                    if "switches" not in _state:
                        raise TypeError(
                            "The argument to states must either be single channel device command (eg: Power.on, Power.off) or atmost two multi-channel device command (eg: (Power.on[0, 1], Power.off[2, 3]) , Power.off[2, 3, 4])."
                        )
            ...
            params = dict(
                ...
                pulse=pulse.name
                if isinstance(pulse, Power)
                else pulse.state.name
                if pulse
                else self.pulse.state.name,
            ...
            self.state = (state or self.state) if _switch.get("switch") else self.state
            ...
            if switches := _switch.get("switches"):
    ...
    def switches(
        self,
    ) -> Subscriptable[int, Callable[[Power], Coroutine[None, Any, None]]]:
