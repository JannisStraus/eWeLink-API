...
class ClientUser:
    ...
    def __init__(
        self,
        data: dict[str, str | bool | dict[str, str | bool | int | Any]],
        http: HttpClient | None,
    ) -> None:
        ...
        self.phone_number: str | None = data.get("phoneNumber")
        self.api_key: str = data.get("apikey")
        self.id: str = data["_id"]
        self.country_code: CountryCodes | None = CountryCodes.__dict__[
            "_value2member_map_"
        ].get(data.get("countryCode", "0"), "0")
        self.online_time = None
        self.offline_time = None
        self.created_at: datetime = datetime.strptime(
            data["createdAt"], "%Y-%m-%dT%H:%M:%S.%fZ"
        )
        self.family_id: str | None = data.get("currentFamilyId")
        if extra := data.get("extra"):
            if ip := extra.get("ipCountry", None):
                self.ip_country: str = ip
        if online_time := data.get("onlineTime"):
            self.online_time: datetime = datetime.strptime(
                online_time, "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        if offline_time := data.get("offlineTime"):
            self.offline_time: datetime = datetime.strptime(
                offline_time, "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        self.online: bool = data.get("online", False)
        if location := data.get("location"):
            self.location = location
        self.http = http
        self.app_infos: list[AppInfo] = [
            AppInfo(
                os=info["os"] if info.get("os", None) else None,
        ...
        self.info: ClientInfo = ClientInfo(
            version=data["clientInfo"]["appVersion"]
            if data["clientInfo"].get("appVersion", None)
            else None,
            imei=data["clientInfo"]["imei"]
            if data["clientInfo"].get("imei", None)
            else None,
            model=data["clientInfo"]["model"]
            if data["clientInfo"].get("model", None)
            else None,
            os=data["clientInfo"]["os"] if data["clientInfo"].get("os", None) else None,
            rom_version=data["clientInfo"]["romVersion"]
